...
char	*handel_varible(char *s, int *i, t_envp **my_env, char *curnt_str)
{
	...
	{
		(*i)++;
		exit_str = ft_itoa(exit_status(-1));
		temp = ft_strjoin(curnt_str, exit_str);
		return (temp);
	}
	var_start = *i;
	...
	if (var_value)
	{
		temp = ft_strjoin(curnt_str, var_value);
		curnt_str = temp;
	}
	return (curnt_str);
}
...
static int	quoted_helper(char *s, int *i, t_quoted_command *data)
{
	char	*temp;
	char	ch[2];
	...
	{
		if (s[*i] == '$' && data->quote == '"')
			data->final_str = handel_env_var(s, i, data->my_env,
					data->final_str);
		else
		{
			ch[0] = s[(*i)++];
