...
✅ No duplicate initialisation of exit status in builtins
✅ Custom implementation of set_env (that always overwrite)
✅ split execute_command and execute_pipeline into small functions to respect 25 lines 
✅ I added execute_builtin(cmd, &env) in the child so builtins inside pipelines run correctly (otherwise they were treated as external commands and failed with “command not found”)
ex : export GHOST=123 | env | grep GHOST 
minishell behavior = command not found 
bash behavior = () -> "means prints nothing when echo $GHOST"